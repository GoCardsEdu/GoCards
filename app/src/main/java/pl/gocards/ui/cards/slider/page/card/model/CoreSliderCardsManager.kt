...
open class CoreSliderCardsManager(
     ...
     * C_C_23 Create a new card
     */
    fun insertNewCardAfter(page: Int, id: Int) {
        insertPageAfter(
            page,
            SliderCardUi(
                id,
    ...
    private suspend fun deleteCardInDb(sliderCard: SliderCardUi) {
        val cards = getItems()
        val page = cards.indexOf(sliderCard)

        require(cards[page].id == sliderCard.id) {
            "Card at index $page doesn't match the provided card's ID."
        }
    ...
    suspend fun restoreDeletedCard(deletedPage: Int, sliderCard: SliderCardUi) {
        ...
        insertPageBefore(deletedPage, sliderCard)
    }
