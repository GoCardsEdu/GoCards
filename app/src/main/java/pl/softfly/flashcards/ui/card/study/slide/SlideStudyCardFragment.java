...
public class SlideStudyCardFragment extends BaseFragment {
    ...
    protected void updateCardLearningProgress(@NonNull CardLearningProgress learningProgress) {
        getModel().updateLearningProgress(
                ...
                .doOnComplete(() -> getStudyActivity().removeCardFromView())
                .subscribe(() -> {}, this::onErrorUpdateCardLearningProgress);
    }
    ...
    protected void onQuickChanged(CardLearningProgress cardLearningProgress) {
        if (cardLearningProgress != null) {
            ...
            getQuickButton().setText("Quick Repetition\n" + displayIntervalWithTime(cardLearningProgress.getInterval()));
        }
    }
    ...
    protected String displayIntervalWithDays(float interval) {
        if (interval < 24 * 60) {
            return displayIntervalWithTime(interval);
    ...
    protected String displayIntervalWithTime(float interval) {
        if (interval < 60) {
    ...
    public void deleteCard() {
        ...
        getDeckDb().cardDao().deleteAsync(card)
                .subscribeOn(Schedulers.io())
                .doOnComplete(() -> {
                    getStudyActivity().removeCardFromView();
    ...
    public SlideStudyCardActivity getStudyActivity() {
        return (SlideStudyCardActivity) getActivity();
    }
