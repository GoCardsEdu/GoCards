...
public class CreateSampleDeck {
    ...
    public void create(@NonNull Context context, Path folder, @NonNull Action doOnComplete)
            throws DatabaseException {
        StorageDb storageDb = DeckDatabaseUtil
                .getInstance(context)
                .getStorageDb();

        String dbPath = folder + "/" + DECK_NAME + ".db";

        if (!storageDb.exists(dbPath)) {
